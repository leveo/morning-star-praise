...
def _detect_staffs_via_llm(image_path: Path, page_idx: int) -> list[StaffBox]:
    ...
    prompt = (
        "You are given a sheet music page. Perform staff detection in TWO "
        "internal reasoning steps, then return ONLY the final JSON.\n\n"
        "STEP 1 — Coarse localization (mental):\n"
        "  Scan the page top-to-bottom and find each \"staff system\" — a row "
        "of 5 parallel horizontal lines containing music notation (may be a "
        "grand staff with treble + bass bracketed together). For each, note "
        "its approximate vertical range AND the vertical range of the lyric "
        "text band directly below it (if lyrics are printed there).\n\n"
        "STEP 2 — Precise refinement (mental):\n"
        "  For each system you located in step 1, examine the exact boundary "
        "between the music and any adjacent text. Determine the precise "
        "y-pixel of:\n"
        "    y_staff_top:    the topmost (5th) staff line\n"
        "    y_staff_bottom: the bottommost (1st) staff line\n"
        "    y_notes_top:    topmost pixel of any notehead, stem, beam, or "
        "ledger line ABOVE the staff (== y_staff_top if none)\n"
        "    y_notes_bottom: bottommost pixel of any notehead, stem, beam, or "
        "ledger line BELOW the staff (== y_staff_bottom if none)\n\n"
        "REQUIREMENTS on y_notes_top / y_notes_bottom:\n"
        "  They must NOT overlap with ANY text glyph. Specifically exclude:\n"
        "  - printed lyrics (Chinese characters, English words, syllables)\n"
        "  - chord symbols (C, G, Dm, F7, etc.)\n"
        "  - verse numbers, copyright, page numbers, section labels\n"
        "  - tempo, dynamics (p, f, crescendo), expression markings\n"
        "  If a notehead sits directly on top of a text line, TIGHTEN the "
        "bound so the text is excluded even if it means cropping a few pixels "
        "of the notehead stem — text contamination is worse than a short stem.\n\n"
        "Coordinates are normalized 0-1000 (0=page top, 1000=page bottom). "
        "Sort systems top-to-bottom.\n\n"
        "Respond with JSON ONLY (no markdown, no prose, no reasoning trace):\n"
